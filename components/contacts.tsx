...
export function Contacts({
  ...
  initialContacts: UserInfromation[];
  initialUnread: string[];
}) {
  const router = useRouter();

  const [contacts] = useState<UserInfromation[]>(initialContacts);
  const [error, setError] = useState<string | undefined>("");
  const [unreadSenders, setUnreadSenders] = useState<string[]>(initialUnread);
  ...
  return (
    <div className="w-screen h-screen bg-gray-800">
      <div className="contacts-container h-screen w-75 flex flex-col gap-4 p-4 bg-gray-900">
